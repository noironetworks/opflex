...
private:
    ...
    /**
     * Parse a notification file
     *
     * @param filePath Path to the notification file
     * @return Parsed notification data or nullptr if parsing failed
     */
    std::unique_ptr<NotificationFile> parseNotificationFile(
        const boost::filesystem::path& filePath);
     ...
     * @return SubscrationType enum value
     */
    SubscrationType parseSubscriptionType(const std::string& typeStr);
