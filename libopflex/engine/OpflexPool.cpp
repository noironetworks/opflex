...
void OpflexPool::updatePeerStatus(const string& hostname, int port,
    ...
    for (PeerStatusListener* l : peerStatusListeners) {
...
void OpflexPool::resetPeers(const bool doConfigured) {
    const std::lock_guard<std::recursive_mutex> lock(conn_mutex);
    conn_map_t conns(connections);
    for (conn_map_t::value_type& v : conns) {
        if ((doConfigured &&
	     isConfiguredPeer(v.first.first, v.first.second)) ||
           (!isConfiguredPeer(v.first.first, v.first.second))) {
            v.second.conn->close();
        }
    }
...
size_t OpflexPool::sendToRole(OpflexMessage* message,
    ...
    for (OpflexClientConnection* conn : it->second.conns) {
        if (!conn->isReady()) continue;
        ready.push_back(conn);
    }
    for (OpflexClientConnection* conn : ready) {
        if (i < (ready.size() - 1)) {
